...
import { Discord, ReHolyNotes } from "./types";
...
export const noteHandler = proxyLazyWebpack(() => {
    class NoteHandler {
        public _formatNote(channel: Channel, message: Message): ReHolyNotes.Note {
            return {
                id: message.id,
        ...
        public getNotes(notebook?: string): Record<string, ReHolyNotes.Note> {
            return noteHandlerCache.get(notebook);
        }

        public getAllNotes(): ReHolyNotes.Note[] {
            const data = noteHandlerCache.entries();
            const notes = {};
            for (const [key, value] of data) {
                notes[key] = value;
            }
            return notes as ReHolyNotes.Note[];
        }
        ...
        public addNote = async (message: Message, notebook: string) => {
            ...
            saveCacheToDataStore(notebook, newNotes as unknown as ReHolyNotes.Note[]);
        ...
        public deleteNote = async (noteId: string, notebook: string) => {
            ...
            saveCacheToDataStore(notebook, lodash.omit(notes, noteId) as unknown as ReHolyNotes.Note[]);
        ...
        public moveNote = async (note: ReHolyNotes.Note, from: string, to: string) => {
            ...
            saveCacheToDataStore(from, lodash.omit(origNotebook, note.id) as unknown as ReHolyNotes.Note[]);
            saveCacheToDataStore(to, newNoteBook as unknown as ReHolyNotes.Note[]);
        ...
        public newNoteBook = async (notebookName: string, silent?: Boolean) => {
            ...
            saveCacheToDataStore(notebookName, {} as ReHolyNotes.Note[]);
        ...
        public refreshAvatars = async () => {
            ...
            for (const notebook in notebooks) {
                ...
                saveCacheToDataStore(notebook, notebooks[notebook] as unknown as ReHolyNotes.Note[]);
            }
        ...
        public importNotes = async (notes: ReHolyNotes.Note[]) => {
            try {
            ...
            for (const notebook in parseNotes) {
                ...
                saveCacheToDataStore(notebook, parseNotes[notebook] as unknown as ReHolyNotes.Note[]);
            }
