...
import { ReHolyNotes } from "./types";

export const ReHolyNoteStore = createStore("ReHolyNoteData", "ReHolyNoteStore");

export async function saveCacheToDataStore(key: string, value?: ReHolyNotes.Note[]) {
    await DataStore.set(key, value, ReHolyNoteStore);
}

export async function deleteCacheFromDataStore(key: string) {
    await DataStore.del(key, ReHolyNoteStore);
}

export async function getFormatedEntries() {
    const data = await DataStore.entries(ReHolyNoteStore);
...
export async function DeleteEntireStore() {
    await DataStore.clear(ReHolyNoteStore);
...
export async function uploadNotes() {
    if (IS_VESKTOP || IS_WEB) {
        ...
        input.onchange = async () => {
            ...
            reader.onload = async () => {
                try {
                    await noteHandler.importNotes(reader.result as unknown as ReHolyNotes.Note[]);
                } catch (err) {
                    console.error(err);
                    Toasts.show({
                        id: Toasts.genId(),
        ...
        if (file) {
            await noteHandler.importNotes(new TextDecoder().decode(file.data) as unknown as ReHolyNotes.Note[]);
        }
    }
}
